...
axis: 막대기가 뻗어나가는 방향과 길이입니다. pos2 - pos1이라고 쓰면 "1번 공에서 2번 공 방향으로 막대기를 연결해라"라는 뜻
결과적으로 두 뼈대 사이를 연결하는 하얀색 염기쌍 막대가 만들어진다
4. 요약
이 코드는 for 반복문을 40번 돌면서 다음과 같은 일을 한다 이번 층은 몇 도 회전하고 얼마나 높이 있을까 계산 왼쪽 끝과 오른쪽 끝에 공을 하나씩 놓자 (뼈대 생성) 두 공 사이에 막대기를 연결 (염기쌍 생성)이걸 40번 반복하면 우리 눈앞에 멋진 3D DNA 구조가 나타나는 것
